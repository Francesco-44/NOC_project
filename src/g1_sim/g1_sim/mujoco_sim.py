...
class MujocoSim(Node):
    def __init__(self):
        ...
        self.viewer = None
        self._viewer_next_t = 0.0
        self._viewer_fps = float(self.get_parameter('viewer_fps').value)
    ...
    def _viewer_sync(self):
        ...
        if fps > 0.0:
            if self.sim_t < self._viewer_next_t:
                return
            self._viewer_next_t = self.sim_t + 1.0 / fps
        self.viewer.sync()
